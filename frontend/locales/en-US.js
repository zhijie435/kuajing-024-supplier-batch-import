export const enUS = {
  app: {
    title: 'Online Course Management System', subtitle: 'Professional Learning Platform',
    home: 'Home', courses: 'Courses', dashboard: 'Dashboard', profile: 'Profile',
    login: 'Login', logout: 'Logout', register: 'Register', search: 'Search courses...',
  },
  course: {
    list: 'Course List', detail: 'Course Detail', price: 'Course Price', original_price: 'Original Price',
    discount_price: 'Discount Price', free: 'Free', enroll: 'Enroll Now', enrolled: 'Enrolled',
    students: 'Students', rating: 'Rating', duration: 'Duration', level: 'Level',
    level_beginner: 'Beginner', level_intermediate: 'Intermediate', level_advanced: 'Advanced',
    category: 'Category', teacher: 'Instructor', start_date: 'Start Date',
    lessons_count: '{count} Lessons',
  },
  currency: {
    label: 'Currency', cny: 'CNY', usd: 'USD', jpy: 'JPY', eur: 'EUR', hkd: 'HKD',
  },
  lang: { label: 'Language', zh_cn: '简体中文', en_us: 'English', ja_jp: '日本語' },
  common: {
    loading: 'Loading...', error: 'Something went wrong', success: 'Operation successful',
    confirm: 'Confirm', cancel: 'Cancel', save: 'Save', delete: 'Delete', edit: 'Edit',
    back: 'Back', more: 'View More', read_more: 'Read More', refresh: 'Refresh Rates',
    no_data: 'No data available', welcome: 'Welcome back, {name}',
    copyright: '© {year} Online Course Management System. All rights reserved.',
  },
  demo: {
    title: 'Multi-Language & Multi-Currency Demo',
    desc_1: 'Switch language and currency above to see real-time changes.',
    desc_2: 'Japanese locale has missing keys, fallback to English then Chinese.',
    desc_3: 'When exchange rate API is unavailable, fallback rates are used.',
    fallback_test: 'Fallback test string (CN only)',
    currency_convert: 'Currency Converter', original_amount: 'Base Amount', converted_amount: 'Converted Amount',
    base_currency: 'Base Currency', target_currency: 'Target Currency', exchange_rates: 'Exchange Rates',
    all_courses: 'All Courses', notes: 'Fallback Demo (Japanese Mode)',
  },
  supplier: {
    title: 'Supplier Management',
    list: 'Supplier List',
    detail: 'Supplier Detail',
    import: 'Batch Import',
    import_title: 'Supplier Batch Import',
    import_template: 'Download Template',
    import_upload: 'Select File',
    import_file: 'Import File',
    import_progress: 'Import Progress',
    import_processing: 'Processing, please wait...',
    import_success: 'Import Successful',
    import_fail: 'Import Failed',
    import_result: 'Import Result',
    import_total: 'Total Rows',
    import_success_count: 'Success',
    import_fail_count: 'Failed',
    import_history: 'Import History',
    import_fail_details: 'Fail Details',
    import_fail_export: 'Export Fail Details',
    import_select_file: 'Please select a CSV file',
    import_file_tip: 'CSV format only, max 10MB. Please download the template first.',
    import_confirm: 'Confirm Import',
    company_name: 'Company Name',
    unified_social_credit_code: 'USCC',
    legal_person: 'Legal Person',
    legal_person_id_card: 'Legal Person ID',
    registered_capital: 'Registered Capital',
    establish_date: 'Establish Date',
    business_scope: 'Business Scope',
    registered_address: 'Registered Address',
    contact_name: 'Contact Name',
    contact_phone: 'Contact Phone',
    contact_email: 'Contact Email',
    status: 'Status',
    status_0: 'Pending',
    status_1: 'Approved',
    status_2: 'Rejected',
    task_status_0: 'Pending',
    task_status_1: 'Processing',
    task_status_2: 'Completed',
    task_status_3: 'Failed',
    search_placeholder: 'Search company name / USCC / contact / phone',
    row_number: 'Row #',
    error_message: 'Error Message',
    operator: 'Operator',
    created_at: 'Created At',
    completed_at: 'Completed At',
    close: 'Close',
    view: 'View',
    refresh: 'Refresh',
    template_download_success: 'Template downloaded successfully',
    template_download_fail: 'Template download failed',
    template_downloading: 'Downloading template...',
    fail_detail_loading: 'Loading fail details...',
    fail_detail_load_success: 'Fail details loaded successfully',
    fail_detail_load_fail: 'Failed to load fail details',
    fail_detail_empty: 'No failed records, all data imported successfully!',
    fail_export_success: 'Fail details exported successfully',
    fail_export_fail: 'Fail details export failed',
    fail_exporting: 'Exporting fail details...',
    operation_logs: 'Operation Logs',
    operation_type: 'Operation Type',
    operation_status: 'Status',
    operation_remark: 'Details',
    operation_time: 'Time',
    operation_ip: 'IP Address',
    op_download_template: 'Download Template',
    op_upload_file: 'Upload File',
    op_process_import: 'Process Import',
    op_view_fail: 'View Fail Details',
    op_export_fail: 'Export Fail Details',
    op_refresh_history: 'Refresh History',
    op_status_success: 'Success',
    op_status_fail: 'Failed',
    view_operation_logs: 'View Logs',
    no_operation_logs: 'No operation logs',
    operation_logs_title: 'Operation Log Details',
  },
};
